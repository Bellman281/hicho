...
/// Bound on the in-flight command queue. Hit counting is best-effort, so once
...
impl HitRecorder for BatchingHitRecorder {
    async fn record(&self, code: ShortCode) {
        ...
        // hit — best-effort counting must never slow or fail a redirect.
        let _ = self.tx.try_send(Msg::Hit(code));
    }
